...
const CreateJobDrive = () => {
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto min-h-screen">
      <button
        onClick={() => navigate(userRole === 'HR' ? '/hr/job-drives' : '/admin/job-drives')}
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="glass-panel p-6 rounded-2xl space-y-6">
          <div className="flex items-center space-x-2 border-b border-[var(--color-border)] pb-4">
            ...
            <h2 className="text-xl font-semibold text-[var(--color-text-primary)]">Eligibility Criteria</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <label className="text-sm font-medium text-[var(--color-text-secondary)]">Minimum CGPA</label>
              <input type="number" step="0.01" min="0" max="10" name="minCgpa" value={formData.minCgpa} onChange={handleChange} className="w-full bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-lg px-4 py-3 text-[var(--color-text-primary)] focus:outline-none focus:border-purple-500 shadow-sm" />
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium text-[var(--color-text-secondary)]">Max Active Backlogs Allowed</label>
              <input type="number" min="0" name="maxBacklogs" value={formData.maxBacklogs} onChange={handleChange} className="w-full bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-lg px-4 py-3 text-[var(--color-text-primary)] focus:outline-none focus:border-purple-500 shadow-sm" />
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium text-[var(--color-text-secondary)]">Target Passout Year <span className="text-red-500">*</span></label>
              <input type="number" name="passoutYear" required value={formData.passoutYear} onChange={handleChange} className="w-full bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-lg px-4 py-3 text-[var(--color-text-primary)] focus:outline-none focus:border-purple-500 shadow-sm" placeholder="e.g. 2024" />
            </div>

            <div className="space-y-2 md:col-span-3">
              <label className="text-sm font-medium text-[var(--color-text-secondary)]">Eligible Branches (Comma separated)</label>
              <input type="text" name="eligibleBranches" value={formData.eligibleBranches} onChange={handleChange} className="w-full bg-[var(--color-bg-primary)] border border-[var(--color-border)] rounded-lg px-4 py-3 text-[var(--color-text-primary)] focus:outline-none focus:border-purple-500 shadow-sm" placeholder="e.g. CSE, IT, ECE (Leave empty for all)" />
            </div>
          </div>
        </div>
