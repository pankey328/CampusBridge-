...
export default function BulkImportStudents() {
  ...
  const handleDryRun = async (dataToValidate = null) => {
    ...
    try {
      ...
      if (dataToValidate) {
        response = await api.post('/admin/students/bulk-import-dryrun', { students: dataToValidate }, {
        ...
        response = await api.post('/admin/students/bulk-import-dryrun', formData, {
          headers: { 
      ...
      if (response.data.errorCount === 0) {
        toast.success(`Perfect! All ${response.data.validCount} records are ready.`);
      } else {
        toast.error(`Found ${response.data.errorCount} issues. Please fix them below.`);
      }
    } catch (error) {
  ...
  const handleRevalidate = () => {
    ...
    const allStudents = [...editableValidStudents, ...editableErrors.map(e => e.data)];
  ...
  const handleCommit = async () => {
    ...
    try {
      ...
      const response = await api.post('/admin/students/bulk-import-commit', payload, {
        headers: { 'Authorization': `Bearer ${token}` }
      });
    ...
    <div className="min-h-screen p-8 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-[var(--color-text-secondary)] mt-2">
          Upload a CSV file. If there are errors (like missing emails), you can fix them right here before importing!
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
        ...
        <div className="xl:col-span-1 space-y-6">
          <div className="glass-panel p-6 rounded-2xl">
            <h3 className="text-lg font-semibold text-white mb-4">1. Upload CSV</h3>
            ...
            >
              ...
              {file ? (
                ...
                <div className="flex flex-col items-center">
                  ...
                  <p className="text-sm text-white font-medium">Drag & Drop your CSV</p>
                </div>
              )}
            </div>
            ...
            {file && !previewData && (
              ...
              >
                {isDryRunning ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Analyze File'}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="xl:col-span-2 space-y-6">
          <div className={`glass-panel p-6 rounded-2xl h-full transition-opacity duration-300 ${previewData ? 'opacity-100' : 'opacity-30'}`}>
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-semibold text-white">2. Review & Fix Errors</h3>
              {previewData && (editableErrors.length > 0 || needsRevalidation) && (
                ...
                >
                  <RefreshCw className={`w-4 h-4 mr-2 ${isDryRunning ? 'animate-spin' : ''}`} />
                  Re-Validate Changes
                </button>
              )}
            </div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-3 gap-4">
                  <div className="bg-[var(--color-bg-input)] p-4 rounded-xl">
                    <p className="text-xs text-[var(--color-text-secondary)] uppercase mb-1">Total Uploaded</p>
                    <p className="text-2xl font-bold text-white">{editableValidStudents.length + editableErrors.length}</p>
                  </div>
                  <div className="bg-emerald-900/20 p-4 rounded-xl border border-emerald-500/30">
                    <p className="text-xs text-emerald-400 uppercase mb-1">Valid (Ready)</p>
                    <p className="text-2xl font-bold text-emerald-400">{editableValidStudents.length}</p>
                  </div>
                  <div className={`p-4 rounded-xl border ${editableErrors.length > 0 ? 'bg-red-900/20 border-red-500/30' : 'bg-[var(--color-bg-input)] border-transparent'}`}>
                    <p className={`text-xs uppercase mb-1 ${editableErrors.length > 0 ? 'text-red-400' : 'text-[var(--color-text-secondary)]'}`}>Issues</p>
                    <p className={`text-2xl font-bold ${editableErrors.length > 0 ? 'text-red-400' : 'text-white'}`}>{editableErrors.length}</p>
                  </div>
                </div>
                  ...
                  <div className="border border-red-500/30 rounded-xl overflow-hidden">
                    <div className="bg-red-900/40 p-3 border-b border-red-500/30">
                      <h4 className="text-sm font-semibold text-red-400 flex items-center">
                        <AlertCircle className="w-4 h-4 mr-2" /> Fix Issues to Import
                      </h4>
                    </div>
                      ...
                      <table className="w-full text-left text-sm relative">
                        ...
                        <tbody className="divide-y divide-red-900/30 bg-[var(--color-bg-dark)]/50">
                          {editableErrors.map((err, idx) => (
                            <tr key={idx}>
                              <td className="px-4 py-3 text-xs text-red-400 max-w-[200px] truncate" title={err.message}>
                                {err.message}
                              </td>
                              <td className="px-2 py-2">
                                <input 
                                  ...
                                  onChange={(e) => handleEditErrorRow(idx, 'enrollmentNumber', e.target.value)}
                              ...
                              <td className="px-2 py-2">
                                <input 
                                  ...
                                  onChange={(e) => handleEditErrorRow(idx, 'email', e.target.value)}
                              ...
                              <td className="px-2 py-2 flex space-x-2">
                                <input 
                                  ...
                                  onChange={(e) => handleEditErrorRow(idx, 'firstName', e.target.value)}
                                ...
                                <input 
                                  ...
                                  onChange={(e) => handleEditErrorRow(idx, 'lastName', e.target.value)}
                                  className="w-20 px-2 py-1 bg-[var(--color-bg-input)] border border-transparent rounded focus:outline-none focus:border-[var(--color-brand-primary)] text-white text-xs"
                                />
                              </td>
                ...
                {editableValidStudents.length > 0 && (
                  <div>
                    <h4 className="text-sm font-semibold text-white mb-3 flex items-center">
                      <CheckCircle2 className="w-4 h-4 mr-2 text-emerald-400" /> Valid Records
                    </h4>
                      ...
                      <table className="w-full text-left text-sm text-[var(--color-text-secondary)] relative">
                        ...
                        <tbody className="divide-y divide-[var(--color-bg-input)]">
                          {editableValidStudents.map((student, idx) => (
                            <tr key={idx} className="bg-[var(--color-bg-dark)]/50 hover:bg-[var(--color-bg-input)]/30 transition-colors">
                              {editingValidRow === idx ? (
                                <>
                                  <td className="px-2 py-2">
                                    <input 
                                      ...
                                      onChange={(e) => handleEditValidRow(idx, 'enrollmentNumber', e.target.value)}
                                  ...
                                  <td className="px-2 py-2">
                                    <input 
                                      type="text" 
                                      value={student.email} 
                                      onChange={(e) => handleEditValidRow(idx, 'email', e.target.value)}
                                  ...
                                  <td className="px-2 py-2">
                                    <input 
                                      ...
                                      onChange={(e) => handleEditValidRow(idx, 'firstName', e.target.value)}
                                  ...
                                  <td className="px-2 py-2">
                                    <input 
                                      ...
                                      onChange={(e) => handleEditValidRow(idx, 'lastName', e.target.value)}
                                      className="w-20 px-2 py-1 bg-[var(--color-bg-dark)] border border-[var(--color-brand-primary)] rounded focus:outline-none text-white text-xs"
                                    />
                                  </td>
                                ...
                                <>
                                  <td className="px-4 py-3 font-mono">{student.enrollmentNumber}</td>
                                  <td className="px-4 py-3">{student.email}</td>
                                  <td className="px-4 py-3 text-white">{student.firstName}</td>
                                  <td className="px-4 py-3 text-white">{student.lastName}</td>
                ...
                <button
                  ...
                  disabled={isUploading || editableValidStudents.length === 0 || needsRevalidation}
                ...
                >
                  {isUploading ? (
                    <>
                      <Loader2 className="w-5 h-5 mr-2 animate-spin" /> Committing Database...
                    </>
                  ) : needsRevalidation ? (
                    <>
                      Pending Edits! Click 'Re-Validate Changes' Before Committing
                    </>
                  ) : (
                    <>
                      Commit & Email {editableValidStudents.length} Valid Students <ArrowRight className="w-5 h-5 ml-2" />
                    </>
                  )}
                </button>
              </div>
